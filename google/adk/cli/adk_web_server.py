...
class AdkWebServer:
  ...
  def get_fast_api_app(
    ...
    state: Optional[dict[str, Any]] = None
      ...
      This endpoint allows updating the state of an existing session. The state
      parameter should contain the new state data that will replace the current
      ...
      Args:
          ...
          state: Optional dictionary containing the new state data to set on the session.
                If None, the session state will remain unchanged.
      
      Returns:
          Session: The updated session object with the new state
          
      Raises:
          HTTPException: 404 if the session is not found
          HTTPException: 500 if there's an error updating the session
          
      Note:
          This operation replaces the entire session state. If you need to update
          only specific keys while preserving others, you should first fetch the
          current session, modify the state locally, then send the complete
          updated state in this request.
      """
      ...
      # Update the session state if provided
      if state is not None:
          try:
              # Create a state delta event to update the session state
              from ..events.event import Event
              ...
              # Apply the state update through the session service
              await self.session_service.append_event(
                  session=session, 
                  event=state_update_event
              )
              
              # Fetch the updated session to return the current state
              ...
              logger.info(
                  "Session state updated: %s for user %s in app %s", 
              ...
              logger.error(
                  "Error updating session %s state: %s", 
                  session_id, str(e)
              )
              raise HTTPException(
                  status_code=500,
      ...
      # If no state update was provided, just return the existing session
      self.current_app_name_ref.value = app_name
      return session
